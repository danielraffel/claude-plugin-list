...
export function extractPluginsFromMarketplace(
  ...
  try {
    ...
    return marketplaceData.plugins.map((plugin) => {
      ...
      const sourceType = typeof plugin.source === "string"
        ? undefined
        : plugin.source.source;
      const sourceRepo = typeof plugin.source === "string"
        ? undefined
        : plugin.source.repo;
      ...
      const hooksRaw = normalizeRecord(plugin.hooks);
      const mcpServersRaw = normalizeRecord(plugin.mcpServers);
      const hooksValue =
        typeof plugin.hooks === "string" || Array.isArray(plugin.hooks)
          ? plugin.hooks
          : undefined;
      const mcpServersValue =
        typeof plugin.mcpServers === "string" || Array.isArray(plugin.mcpServers)
          ? plugin.mcpServers
          : undefined;
      ...
      const transformedPlugin: Plugin = {
        ...
        hooks: normalizeToArray(hooksValue as string | string[] | undefined),
        hooksRaw,
        mcpServers: normalizeToArray(mcpServersValue as string | string[] | undefined),
        mcpServersRaw,
        installCommand,
      };
